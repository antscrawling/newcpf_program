...
class CPFAccount:
    ...
    def excess_balance(self, data):
        ...
        log_entry = {
            ...
            'amount': diff,
            'type': 'inflow' if diff > 0 else ('outflow' if diff < 0 else 'no change'),
            'message': f'excess-{message}-{diff:.2f}'
        }
    ...
    def loan_balance(self, data):
        ...
        log_entry = {
            ...
            'amount': diff,
            'type': 'inflow' if diff > 0 else ('outflow' if diff < 0 else 'no change'),
            'message': f'loan-{message}-{diff:.2f}'
        }
    ...
    def record_inflow(self, account: str, amount: float, message: str = None):
        ...
        setattr(self, f"{account}_balance", (new_balance, message or "inflow"))
    ...
    def record_outflow(self, account: str, amount: float, message: str = None):
        ...
        # Use the property setter to update balance and trigger logging
        setattr(self, f"{account}_balance", (new_balance, message or "outflow"))
    ...
    def apply_interest(self, age: int):
        """Applies interest to OA, SA, MA, RA based on configuration rates."""
        # Get current balances (using the getter part of the property is fine here)
        current_oa, _ = self.oa_balance
        current_sa, _ = self.sa_balance
        current_ma, _ = self.ma_balance
        current_ra, _ = self.ra_balance

        # Get interest rates from config (example structure)
        rates = self.config.get('interest_rates', {})
        oa_rate = rates.get('oa', 0.025)
        sa_rate = rates.get('sa', 0.04)
        ma_rate = rates.get('ma', 0.04)
        ra_rate = rates.get('ra', 0.04)
        # Add logic for extra interest if applicable based on config/age

        # Calculate interest amounts (simplified - actual calculation might be monthly compounded)
        oa_interest = current_oa * (oa_rate / 12) # Assuming monthly interest application
        sa_interest = current_sa * (sa_rate / 12)
        ma_interest = current_ma * (ma_rate / 12)
        ra_interest = current_ra * (ra_rate / 12)

        # --- CRITICAL PART ---
        # Update balances using the property setters to trigger logging
        if oa_interest > 0:
            self.oa_balance = (current_oa + oa_interest, f"Interest Earned ({oa_rate*100}%)")
        if sa_interest > 0:
            self.sa_balance = (current_sa + sa_interest, f"Interest Earned ({sa_rate*100}%)")
        if ma_interest > 0:
            self.ma_balance = (current_ma + ma_interest, f"Interest Earned ({ma_rate*100}%)")
        if ra_interest > 0:
            self.ra_balance = (current_ra + ra_interest, f"Interest Earned ({ra_rate*100}%)")

        # Add logic for extra interest application here, also using setters

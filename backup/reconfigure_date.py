...
class MyDateTime:
    ...
    def check_type(self): 
        ...
        if isinstance(self.mydate, str):
            return True
        elif isinstance(self.mydate, datetime):
            return False
        else:
            raise TypeError(f"Expected a string or datetime object, got {type(self.mydate)}")
    ...
    def convert_date(self) -> datetime:
        ...
        parsed_date = self.parse_date(self.mydate) if self.check_type() else None
        if parsed_date:
            return parsed_date

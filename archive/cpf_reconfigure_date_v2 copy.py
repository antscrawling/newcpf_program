...
class MyDateTime:
    ...
    def check_string(self): 
        """Checks if the input date is a string or datetime object."""
        if isinstance(self.mydate, str):
            return True
        else: return False
        
    def check_datetime(self):
        """Checks if the input date is a datetime object."""
        if isinstance(self.mydate, datetime):
            return True
        else: return False
    ...
    def convert_date(self) -> datetime:
        ...
        elif self.check_string() and self.parse_date(self.mydate):
            return self.parse_date(self.mydate).date()
        else:    
            ...
            if len(parts) == 3:
                try:
                    ...
                    if len(parts[0]) > 4:
                        ...
                        month = self.months_abbr.get(parts[0][3:] )
...
if __name__ == "__main__":
    ...
    dates = ['2025-04-01','2055-07-31', '1974-07-06']
     # "31/12/1999", datetime(2029,12,20), "20291224", "decyembre 6, 1974", "Januari 15, 2023",
    ...
    for date in dates:
        try:
            result = MyDateTime(date).convert_date()
            print(f"Input Date: {date=} | Converted: {result=}  {type(result)=}")
